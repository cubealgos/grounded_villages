...
| `tools/icon.py` | Render docs/modrinth/icon.png: the village bell on the cubealgos navy badge (GV-23). | `bell_elements()`, `ground_slab_element()`, `frame_elements(jar)`, `rot_axis(p, axis, deg)`, `rotate_about(p, origin, axis, deg)`, `display_transform(p, pivot, rx, ry, rz)`, `affine_from_points(src, dst)`, `invert_affine(coef)`, `build_faces(model, textures, pivot, gui)`, `fit_scale(faces, canvas, margin)`, `render_model(model, textures, gui)`, `badge(grid)`, `compose(base, sprite, box)`, `compose_pixelart(base, raw, box)`, `jar_has_entry(jar, entry)`, `find_jar(explicit)`, `load_texture(jar, entry)`, `load_cropped_sprite(jar, entry)`, `render_bell(textures, gui)`, `render_bell_on_slab(textures, gui)`, `render_bell_with_frame(textures, gui, jar)`, `build_candidates(jar)`, `compose_candidate(sprite, mode)`, `write_sheet(candidates, path)`, `main()` |
