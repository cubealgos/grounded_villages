...
| `tools/target_notes.py` | GV-20: per-target Modrinth release notes, one `dist/notes/.md` per `docs/modrinth/targets.json` entry (GV-19's twelve Stonecutter-node targets). | `load_targets(path)`, `changelog_section(text)`, `tested_line(info)`, `wave_statement(info)`, `notes_for(target, changelog_body)`, `write_all(targets, changelog_body, out_dir)`, `main(argv)` |
