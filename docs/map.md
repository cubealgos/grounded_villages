...
| `tools/target_notes.py` | GV-20: per-target Modrinth release notes, one `dist/notes/.md` per `docs/modrinth/targets.json` entry (GV-19's twelve Stonecutter-node targets). | `load_targets(path)`, `changelog_section(text)`, `tested_line(info)`, `wave_statement(info)`, `load_checksums(path)`, `files_line(target, checksums)`, `notes_for(target, changelog_body, checksums)`, `write_all(targets, changelog_body, out_dir, checksums)`, `main(argv)` |
...
| `tools/test_target_notes.py` | tools/target_notes.py, exercised both as a library (the pure per-target formatting) and as the command a person runs (rule 6 of the standard): a fixture repo tree in a temporary directory, a real `docs/modrinth/targets.json` subset, a real `CHANGELOG.md`, `python3 tools/target_notes.py` against it, then the written `dist/notes/*.md` files read back. | `make_target(node, version_number)` |
