...
| `tools/icon.py` | Render docs/modrinth/icon.png: the village bell on the cubealgos navy badge Create-family add-ons share (GV-23). | `bell_elements()`, `ground_slab_element()`, `rot_axis(p, axis, deg)`, `rotate_about(p, origin, axis, deg)`, `display_transform(p, pivot, rx, ry, rz)`, `affine_from_points(src, dst)`, `invert_affine(coef)`, `build_faces(model, textures, pivot, gui)`, `fit_scale(faces, canvas, margin)`, `render_model(model, textures, gui)`, `badge()`, `compose(base, sprite, box)`, `jar_has_entry(jar, entry)`, `find_jar(explicit)`, `load_texture(jar, entry)`, `render_bell(textures, gui)`, `render_bell_on_slab(textures, gui)`, `build_candidates(jar)`, `write_sheet(candidates, path)`, `main()` |
